...
# Plotting sine function with blue star points
...
legend("bottomright", inset=-0.5, lty=1, legend=c("Sine", "Cos"), 
       col=c("blue", "red"), pch=c(8, 4), title="Sine/Cos Function", xpd=TRUE)
...
# Create the bar plot
bp <- barplot(probability, names.arg = programs, col = "gray", border = "black",
              xlab = expression(italic(x) ~ "(number of assistance programs)"), 
              ylab = "Probability", 
              ylim = c(0, 0.25), space = 0.5, axes=FALSE)

# Adding X-axis with inward ticks
axis(1, at = bp, labels = programs, line = 0, col = "black", tck = 0.02)

# Adding Y-axis with inward ticks
axis(2, at=seq(0,0.25,0.05), tck = 0.02, las=1)

# Explicitly add the X-axis line at y=0 to connect both the axis
...
legend("topleft", inset=0.07, lty="dashed", legend="Per curve", 
       col="pink", cex=0.8)

# Function to calculate factorial
fact <- function(x) {
  if (x == 0) return(1)  # 0! is always 1
  return(prod(1:x))  # Multiply all numbers from 1 to x
}

# Function to calculate combinations (Cn = "n choose k")
Cn <- function(n, k) {
  return(fact(n) / (fact(k) * fact(n - k)))
}

# Function to calculate Hypergeometric probability
hypergeom_pmf <- function(k, N, K, n) {
  return((Cn(K, k) * Cn(N-K, n-k)) / Cn(N, n))
}
...
#  Compute probabilities
x_vals <- 0:min(K, n)  # All possible values of k
probabilities <- sapply(x_vals, function(k) hypergeom_pmf(k, N, K, n))

# Plot the histogram
barplot(probabilities, names.arg=x_vals, col="lightblue", border="black",
        main="Hypergeometric Distribution (Manual Calculation)", 
        xlab="Number of Successes", ylab="Probability")
